...
interface OTPVerificationModalProps {
  ...
  onVerified: (firebaseIdToken: string) => void; // Callback with Firebase ID token
  title?: string;
  description?: string;
}
...
export function OTPVerificationModal({
  ...
  useEffect(() => {
    if (isOpen) {
      const verifier = phoneAuthHelpers.setupRecaptcha('otp-verify-recaptcha');
      setRecaptchaVerifier(verifier);

      // Auto-send OTP when modal opens
      setTimeout(() => {
        handleSendOTP(verifier);
      }, 300);
  ...
  const handleSendOTP = async (verifier?: any) => {
    setInlineError('');
    setSending(true);

    try {
      const result = await phoneAuthHelpers.sendOTP(
      ...
      if (result.success && result.confirmationResult) {
        ...
        if (errorMsg.includes('too-many-requests') || errorMsg.includes('quota')) {
          setInlineError('Too many attempts. Please wait and try again later.');
        } else {
  ...
  const handleVerifyOTP = async () => {
    ...
    if (!confirmationResult) {
      setInlineError('No OTP session found. Please request a new OTP.');
      return;
    }
    ...
    try {
      ...
      if (result.success && result.firebaseIdToken) {
        success('Phone verified successfully!');
        onVerified(result.firebaseIdToken);
        handleClose();
      } else {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        ...
        >
          ...
          <div className="p-6 space-y-6">
            ...
            <div id="otp-verify-recaptcha" className="flex justify-center"></div>
