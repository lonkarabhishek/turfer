...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Build query with turf and host information
      let query = supabase
        .from('games')
        .select(`
          *,
    ...
    try {
      ...
      // Note: creator_id references auth.users, but phone/PIN users are in users table
      // So we set creator_id to NULL and use host_name/host_phone instead
      const insertData = {
        creator_id: null, // Can't use userId because FK references auth.users, not users table
        ...
        skill_level: gameData.skillLevel || 'beginner',
        max_players: gameData.maxPlayers,
        current_players: 1,
        price_per_player: gameData.costPerPerson,
        status: 'open',
        host_name: userProfile.name || 'User',
        host_phone: userProfile.phone || '9999999999',
        host_profile_image_url: userProfile.profile_image_url || '',
      ...
      const transformedGame = {
        id: newGame.id,
        hostId: userId, // Use the authenticated user's ID
        ...
        costPerPerson: newGame.price_per_player,
        description: newGame.description,
        hostName: newGame.host_name,
        hostPhone: newGame.host_phone,
        joinRequests: [],
        confirmedPlayers: [],
