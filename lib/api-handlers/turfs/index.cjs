...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (sport) {
      // Capitalize first letter to match database format (e.g., "cricket" -> "Cricket")
      const capitalizedSport = sport.charAt(0).toUpperCase() + sport.slice(1).toLowerCase();
      // Use overlaps instead of contains for JSONB array matching
      supabaseQuery = supabaseQuery.overlaps('sports', [capitalizedSport]);
    }
